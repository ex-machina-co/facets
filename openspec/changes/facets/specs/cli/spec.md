...
A developer SHALL be able to run a single command to register the facets plugin with their OpenCode project and initialize the structure needed to start using facets.

#### Scenario: Project set up successfully
- **WHEN** a developer runs the setup command in a project
- **THEN** the system SHALL configure OpenCode to load the facets plugin and create the necessary project structure
