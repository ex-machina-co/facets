...
- [ ] 1.2 Propose: Present planned `src/` module structure, dual entry points (CLI + plugin), and runtime dependencies for approval
- [ ] 1.3 Implement: Configure `package.json` with dual entry points and runtime deps (`@opencode-ai/plugin`, `js-yaml`, `zod`, `comment-json`)
- [ ] 1.4 Implement: Scaffold `src/` directory structure
...
- [ ] 2.3 Implement: Define and export `FacetManifest` Zod schema (`facet.yaml` shape — name, version, description, author, requires, skills, agents, commands, platforms)
...
- [ ] 4.3 Implement: Implement `installFacet(name, projectRoot)` — resolves local or cached facet, copies resources to type-specific destinations (skills, agents with assembled frontmatter, commands with assembled frontmatter, tools)
...
## 5. Plugin: OpenCode Tool Surface
...
- [ ] 5.3 Implement: Implement `facet-list` tool — calls `listFacets`, returns structured facet list (name, version, description, installed status)
- [ ] 5.4 Implement: Implement `facet-install` tool — calls `installFacet`, returns success confirmation or failure reason
- [ ] 5.5 Implement: Implement `facet-add` tool — calls `cacheFacet` with agent-provided URL, returns resolved name and version
- [ ] 5.6 Implement: Implement `facet-update` tool — re-fetches cached remote facet, reports new version or "already current"
- [ ] 5.7 Implement: Implement `facet-remove` tool — calls `uninstallFacet`, returns confirmation
- [ ] 5.8 Implement: Register all five tools with the OpenCode plugin entry point using `@opencode-ai/plugin`
...
- [ ] 6.3 Implement: Implement `init` command — registers facets plugin in `.opencode/opencode.jsonc`, creates `facets.yaml` if absent; idempotent with confirmation on re-run
...
- [ ] 7.2 Verify: Type check passes (`bun run typecheck`)
- [ ] 7.3 Verify: All tests pass (`bun test`)
