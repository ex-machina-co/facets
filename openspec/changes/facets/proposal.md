...
- **New package**: `@ex-machina/facets` — an npm package providing a facet engine, OpenCode plugin, and CLI
...
- **New CLI**: `bunx @ex-machina/facets <command>` for terminal-driven facet management (`install`, `init`, `add`, `remove`, `update`, `list`)
...
- New top-level package at `facets/` (this repo)
