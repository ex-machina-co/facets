...
The MCP server surface — tools exposed to any MCP-compatible AI client for managing facets in-session.

## Requirements

### Requirement: AI agents can discover available facets
An AI agent in any MCP-compatible session SHALL be able to retrieve the list of all facets declared by the project — local and remote — including whether each is currently installed.
